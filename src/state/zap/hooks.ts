...
export function useDerivedZapInfo(
  ...
  const isTradingCurrency0 =
    ...
    (currency0?.symbol === WNATIVE[ChainId.METIS].symbol && currency === Ether.onChain(ChainId.METIS))
  const isTradingCurrency1 =
    currency?.symbol === currency1?.symbol ||
    (currency1?.symbol === WNATIVE[ChainId.METIS].symbol && currency === Ether.onChain(ChainId.METIS))
