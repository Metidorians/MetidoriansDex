...
export function useMultichainCurrencyBalance(
  ...
  const { chainId: metisChainId } = useActiveWeb3React()
  const metisBalance = useCurrencyBalance(
    chainId == metisChainId && account,
    chainId == metisChainId && currency
  )
  ...
  useEffect(() => {
    if (account && chainId && currency && chainId != metisChainId) {
      getBalance()
    } else {
      setValue(null)
    }
  }, [account, chainId, currency, getBalance, metisChainId])

  return useMemo(() => {
    return chainId == metisChainId ? metisBalance : value
  }, [chainId, metisBalance, metisChainId, value])
}
