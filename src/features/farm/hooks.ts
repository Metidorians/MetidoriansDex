...
import { METIDORIANS, MASTERCHEF_V2_ADDRESS, MINICHEF_ADDRESS, USDC } from '../../constants'
import { NEVER_RELOAD, useSingleCallResult, useSingleContractMultipleData } from '../../state/multicall/hooks'
import { useCallback, useEffect, useMemo, useState } from 'react'
import {
  useMetidoriansDistributorContract,
  useBNBPairContract,
  useMetidoriansMetisContract,
  useMetidoriansVaultContract,
  useMovrUsdcContract,
  useRibMovrContract,
  useDarkMatterContract,
} from '../../hooks'
...
export function useChefContract(chef: Chef) {
  const solarDistributorContract = useMetidoriansDistributorContract()
...
export function usePendingSolar(farm) {
  ...
  return amount ? CurrencyAmount.fromRawAmount(METIDORIANS[chainId], amount) : undefined
}
...
export function usePositions() {
  return useSolarPositions(useMetidoriansDistributorContract())
}
...
export function useFarms() {
  return useSolarFarms(useMetidoriansDistributorContract())
}


export function usePricesApi() {
  const metisPrice = useMetisPrice()
  const metidoriansPrice = usemetidoriansPrice()
  const ribPrice =  useDarkMatterPrice2()
  const dmtPrice = useDarkMatterPrice() * useMetisPrice()
  

  return useMemo(() => {
    return {
      metis: metisPrice,
      metidorians: metidoriansPrice * metisPrice,
      rib: ribPrice,
      dmt: dmtPrice,
      usdc: 1,
     
    }
  }, [metisPrice, ribPrice, metidoriansPrice, dmtPrice])
}
...
export function usemetidoriansPrice() {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return usePrice(useMetidoriansMetisContract())
}
...
export function useDarkMatterPrice() {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return usePrice(useDarkMatterContract(), 0,false)
}

export function useDarkMatterPrice2() {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return usePrice(useDarkMatterContract(), 0,true)
}
...
export function useDistributorInfo() {
  return useSolarDistributorInfo(useMetidoriansDistributorContract())
}
