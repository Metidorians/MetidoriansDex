...
import { SOLAR_DISTRIBUTOR_ADDRESS, METIDORIANS_VAULT_ADDRESS } from '../../constants/addresses'
...
const VaultListItem = ({ farm }) => {
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, METIDORIANS_VAULT_ADDRESS[chainId])
    ...
    <>
      <Modal isOpen={showConfirmation} onDismiss={() => setShowConfirmation(false)}>
        <div className="space-y-4">
          ...
          />
          <Typography variant="lg" className="pt-4 font-medium">
            {i18n._(
              t`${
                currentAction.action == 'deposit' ? 'Staking on Asteroid Belt' : 'Harvesting'
              } will lock your tokens for ${farm?.lockupDuration / 86400 } Days.`  
            )}
          </Typography>
          <Typography variant="sm" className="pb-4 mt-2 font-medium">
              ...
              if (
                window.prompt(
                  i18n._(
                    ...
                    } and lock your tokens for ${farm?.lockupDuration / 86400} Days`
                  )
                ) === 'confirm'
              ) {
          ...
          >
            <Typography variant="lg">
              {i18n._(t`${currentAction.action == 'deposit' ? 'Stake' : 'Harvest'} & Lock for ${farm?.lockupDuration / 86400} Days`)}
            </Typography>
          </Button>
        </div>
      </Modal>
      <Transition
        ...
        leaveTo="opacity-0"
      >
        <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800/20" static>
          <div className="grid grid-cols-2 gap-4 p-4">
            <div className="col-span-2 text-center md:col-span-1">
              ...
              <div className="relative flex items-center w-full mb-4">
                <NumericalInput
                  className="w-full px-4 py-4 pr-20 rounded bg-dark-700/20 focus:ring focus:ring-dark-purple"
                  value={depositValue}
          ...
          {pendingSolar && pendingSolar.greaterThan(ZERO) && (
            <div className="px-4 pb-4">
              <Button
                ...
                onClick={async () => {
                  ...
                  if (farm?.lockupDuration == 0) {
                    ...
                    setCurrentAction({
                      action: 'harvest',
                      lockup: `${farm?.lockupDuration / 1} Days`,
                      callback: fn,
                    })
                    setShowConfirmation(true)
                  }
              ...
              >
                {i18n._(t`Harvest ${formatNumber(pendingSolar.toFixed(18))} Dark Matter`)}
              </Button>
            </div>
          )}
        </Disclosure.Panel>
