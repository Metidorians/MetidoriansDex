...
import { METIDORIANS_ADDRESS, FACTORY_ADDRESS, SOLAR_DISTRIBUTOR_ADDRESS, METIDORIANS_VAULT_ADDRESS } from '../constants'
...
export function useVaultTVL(): TVLInfo[] {
  ...
  const farmingPools = Object.keys(VAULTS[ChainId.METIS]).map((key) => {
    return { ...VAULTS[ChainId.METIS][key] }
  })
  ...
  const distributorBalance = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'balanceOf', [
    METIDORIANS_VAULT_ADDRESS[ChainId.METIS],
  ])
  ...
  return useMemo(() => {
    function isKnownToken(token: TokenInfo) {
      return (
        token.id.toLowerCase() == METIDORIANS_ADDRESS[chainId].toLowerCase() ||
        token.symbol == 'WMETIS' ||
        token.symbol == 'METIS' ||
        token.symbol == 'DMT' ||
        token.symbol == 'USDC' ||
    ...
    function getPrice(token: TokenInfo) {
      if (token.id.toLowerCase() == METIDORIANS_ADDRESS[chainId].toLowerCase()) {
        return metidoriansPrice
      }
      if (token.symbol == 'WMOVR' || token.symbol == 'MOVR') {
        return metisPrice
      }
      if (token.symbol == 'DMT' || token.symbol == 'DMT') {
        return ribPrice
      }
...
export function useTVL(): TVLInfo[] {
  ...
  const farmingPools = Object.keys(POOLS[ChainId.METIS]).map((key) => {
    return { ...POOLS[ChainId.METIS][key], lpToken: key }
  })
  ...
  const totalSupply = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'totalSupply')
  const distributorBalance = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'balanceOf', [
    SOLAR_DISTRIBUTOR_ADDRESS[ChainId.METIS],
  ])
  const distributorBalanceSingle = useMultipleContractSingleData(singleAddresses, PAIR_INTERFACE, 'balanceOf', [
    SOLAR_DISTRIBUTOR_ADDRESS[ChainId.METIS],
  ])
  ...
  return useMemo(() => {
    ...
    function getPrice(token: TokenInfo) {
      ...
      if (token.symbol == 'WMETIS' || token.symbol == 'METIS') {
        return metisPrice
      }
      if (token.symbol == 'DMT' || token.symbol == 'DMT') {
        return ribPrice
...
export function useV2PairsWithPrice(
  ...
  const priceData = useContext(PriceContext)
  const metidoriansPrice = priceData?.['metidorians']
  const metisPrice = priceData?.['movr']
  const ribPrice = priceData?.['rib']

  return useMemo(() => {
    function isKnownToken(token: Token) {
      return (
        token.address.toLowerCase() == METIDORIANS_ADDRESS[chainId].toLowerCase() ||
        token.symbol == 'WMETIS' ||
        token.symbol == 'METIS' ||
        token.symbol == 'DMT' ||
        token.symbol == 'USDC' ||
        token.symbol == 'MMDORIANS'
      )
    }

    function getPrice(token: Token) {
      if (token.address.toLowerCase() == METIDORIANS_ADDRESS[chainId].toLowerCase()) {
        return metidoriansPrice
      }
      if (token.symbol == 'WMETIS' || token.symbol == 'METIS') {
        return metisPrice
      ...
      if (token.symbol == 'USDC' || token.symbol == 'USDC') {
        return 1
      }
      return 0
    }
  ...
  }, [results, chainId, metidoriansPrice, metisPrice, ribPrice, tokens, totalSupply])
}
