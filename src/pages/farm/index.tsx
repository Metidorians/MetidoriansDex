...
export default function Farm(): JSX.Element {
  ...
  const metidoriansPrice = priceData?.['solar']
  ...
  let summTvlVaults = vaults.reduce((previousValue, currentValue) => {
    return previousValue + (currentValue.totalLp / 1e18) * metidoriansPrice
  }, 0)
  ...
  const FILTER = {
    ...
    metidorians: (farm) => farm.pair.token0?.id == METIDORIANS_ADDRESS[chainId] || farm.pair.token1?.id == METIDORIANS_ADDRESS[chainId],
  ...
  const allStaked = positions.reduce((previousValue, currentValue) => {
    return previousValue + (currentValue.pendingSolar / 1e18) * metidoriansPrice
  }, 0)
  ...
  return (
    <>
      <Head>
        <title>Farm | Metidorians</title>
        <meta key="description" name="description" content="Farm Dark Matter" />
      </Head>
                ...
                <div className={`grid grid-cols-12 md:space-x-4 space-y-4 md:space-y-0 `}>
                  <div className={`col-span-12 md:col-span-3 space-y-4`}>
                    ...
                    <div className={`flex flex-col items-center justify-between px-6 py-6 `}>
                      <div className="flex items-center justify-between py-2 text-center text-emphasis">
                          ...
                          onClick={async () => {
                            ...
                            for (const pos of positions) {
                              try {
                                ...
                                addTransaction(tx, {
                                  summary: `${i18n._(t`Harvest`)} Dark Matter`,
                                })
                              } catch (error) {
                                console.error(error)
                              }
                  ...
                  <div className={`col-span-12 md:col-span-9 py-4 md:px-6 md:py-4 rounded`}>
                    <div className={'mb-8 px-1 md:px-0'}>
                      <Search
                        className={'bg-dark-800/20 rounded border border-dark-800/20'}
