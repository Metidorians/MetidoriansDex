...
export async function farms() {
  if (!farmsResult) {
    let distributorContract = new web3.eth.Contract(distributorAbi, '0x56F6677F1932b51FC1256e4925F9666F62FaD151')
    ...
    const ret = []
    const poolStaticInfo = POOLS[ChainId.METIS]
    ...
    for (const pool of poolsInfo) {
      ...
      if (!staticInfo.token1) {
        ret.push({
          ...
          baseSymbol: staticInfo.token0.symbol.toLowerCase().replace('wmetis', 'metis'),
        ...
        const promisesCall = [
          ...
          lpContract.methods.balanceOf('0x56F6677F1932b51FC1256e4925F9666F62FaD151').call(), //DISTRIBUTOR
        ]
        ...
        ret.push({
          ...
          baseSymbol:
            token0info.symbol == 'SOLAR' || token1info.symbol == 'SOLAR'
              ? 'solar'
              : token0info.symbol == 'METIS' || token1info.symbol == 'METIS'
              ? 'movr'
